...
// 1. Insert Rekapitulasi content before DESKTOP TABLE VIEW
const desktopTableMarker = `{/* DESKTOP: TABLE VIEW (hidden on mobile) */}`;
const rekapContent = `{pmMainTab === 'Rekapitulasi' ? (() => {
    ...
    <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-xs border-collapse">
          ...
          <tbody>
            {topSchools.map(([name, count]) => { const sL = students.filter(s => s.schoolName === name && s.jk === 'L').length; const sP = students.filter(s => s.schoolName === name && s.jk === 'P').length; return (<tr key={name} className="border-b border-slate-100 hover:bg-slate-50/50"><td className="px-3 py-2 font-medium text-slate-700">{name}</td><td className="px-3 py-2 text-center font-bold">{count}</td><td className="px-3 py-2 text-center text-blue-500">{sL}</td><td className="px-3 py-2 text-center text-pink-400">{sP}</td></tr>); })}
...
code = code.replace(desktopTableMarker, rekapContent);

// 2. Wrap desktop table div
const oldDesktopDiv = `<div className="hidden md:block bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">`;
const newDesktopDiv = `{pmMainTab !== \'Rekapitulasi\' && <div className="hidden md:block bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">`;
code = code.replace(oldDesktopDiv, newDesktopDiv);

// 3. Wrap mobile cards div
const oldMobileDiv = `<div className="md:hidden space-y-3">`;
const newMobileDiv = `{pmMainTab !== \'Rekapitulasi\' && <div className="md:hidden space-y-3">`;
code = code.replace(oldMobileDiv, newMobileDiv);

fs.writeFileSync(filePath, code, 'utf-8');
console.log('OK - Rekapitulasi tab patched successfully');
